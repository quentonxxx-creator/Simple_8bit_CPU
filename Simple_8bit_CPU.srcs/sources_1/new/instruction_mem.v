...
module instruction_mem(
    ...
    initial begin
        ...
        rom[6]  = 16'h8001; // BEQ  R0,R0,+1 → 跳到8
        rom[7]  = 16'hF000; // NOP (被跳過)
        rom[8]  = 16'h2712; // AND  R7=R1&R2 → 2
        rom[9]  = 16'h3812; // OR   R8=R1|R2 → 11
        rom[10] = 16'h4912; // XOR  R9=R1^R2 → 9
        rom[11] = 16'hA305; // STORE MEM[R0+5] ← R3 (13)
        rom[12] = 16'hA106; // STORE MEM[R0+6] ← R1 (10)
        rom[13] = 16'h9A05; // LOAD  RA ← MEM[R0+5] → 13
        rom[14] = 16'hF000; // NOP
        rom[15] = 16'h9B06; // LOAD  RB ← MEM[R0+6] → 10
        rom[16] = 16'hF000; // NOP
        rom[17] = 16'hF000; // NOP
        rom[18] = 16'h0CAB; // ADD   RC=RA+RB → 23
    end
